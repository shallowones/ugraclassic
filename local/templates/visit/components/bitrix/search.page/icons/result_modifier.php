...
foreach($arResult["SEARCH"] as $i=>$arItem)
{
	...
	// Модифицируем URL материала, если найден #COLL_CODE#

    if(strpos($arItem['URL_WO_PARAMS'], '#COLL_CODE#') !== false)
    {
        $arCollEl = \CIBlockElement::GetList(
            [],
            ['IBLOCK_ID'=>$arItem['PARAM2'],'ID' => $arItem['ITEM_ID']],
            false,false,
            ['IBLOCK_SECTION_ID']
        )->GetNext();

        if($arCollEl['IBLOCK_SECTION_ID'])
        {
            $arCollSec = \CIBlockSection::GetList(
                [],
                ['IBLOCK_ID'=>$arItem['PARAM2'], 'ID' => $arCollEl['IBLOCK_SECTION_ID']],
                false,
                ['UF_COLLECTIVE'])->GetNext();

            if($arCollSec['UF_COLLECTIVE'])
            {
                $arColl = \CIBlockElement::GetList(
                    [],
                    ['IBLOCK_ID'=>$ibCollID,'ID' => $arCollSec['UF_COLLECTIVE']],
                    false,false,
                    ['ID', 'NAME', 'CODE']
                )->GetNext();
                ...
                if($arColl['ID'])
                {
                    $arResult["SEARCH"][$i]['URL_WO_PARAMS'] =
                        str_replace('#COLL_CODE#', $arColl['CODE'], $arItem['URL_WO_PARAMS']);
...
//\UW\SystemBase::debug($arResult["SEARCH"]);
