...
<form action="<?=POST_FORM_ACTION_URI?>" method="POST" enctype="multipart/form-data" class="feedback-form">
    ...
    <div class="f-line">
        <div class="tf-block">
            ...
            <input class="form__text" type="text" id="fio" name="user_name" value="<?=$arResult["AUTHOR_NAME"]?>">
        </div>
        <div class="tf-block">
            ...
            <input class="form__text" type="email" id="email" name="user_email" value="<?=$arResult["AUTHOR_EMAIL"]?>" <? if($arResult["ERROR_MESSAGE"]['EMAIL']): ?>class="err"<? endif; ?>>
        </div>
    </div>
    <div class="tf-block">
        ...
        <textarea class="form__text" id="message" name="MESSAGE" <? if($arResult["ERROR_MESSAGE"]['MESSAGE']): ?>class="err"<? endif; ?>><?=$arResult["MESSAGE"]?></textarea>
    </div>
        ...
        <?if($arParams["USE_CAPTCHA"] == "Y"):?>
            <div class="cap-block">
                ...
                <div class="feedback-cod">
                    ...
                    <input type="text" class="form__text" name="captcha_word" size="30" maxlength="50" value="" <? if($arResult["ERROR_MESSAGE"]['CAPTCHA']): ?>class="err"<? endif; ?>>
                </div>
            </div><br>
        <?endif;?>
        <div class="feedback-button">
            <input type="hidden" name="PARAMS_HASH" value="<?=$arResult["PARAMS_HASH"]?>">
            <input type="submit" class="btn form__submit" name="submit" value="<?=GetMessage("MFT_SUBMIT")?>">
        </div>
