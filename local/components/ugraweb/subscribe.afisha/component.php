...
while($arIBlock = $rsIBlock->Fetch())
{
	...
	while($obNews = $rsNews->GetNextElement())
	{
        ...
        $arProp = $obNews->GetProperties();

		// генерация выпуска подписки происходит один раз для всех подписчиков
		// новых элементов может быть не более 10, поэтому допускаем подзапросы внутри цикла
		$arEA = CIBlockElement::GetList(
			[],
			[
				'IBLOCK_TYPE' => 'afisha',
				'IBLOCK_CODE' => 'events',
                'ID' => $arProp['event_id']['VALUE']
			],
			false,
			false,
			[
		...
		if($arEA['ID'])
		{
            $arNews['DETAIL_PAGE_URL'] 		= $arEA['DETAIL_PAGE_URL'];
            $arNews['PREVIEW_PICTURE'] 		= $arEA['PREVIEW_PICTURE'];
            $arNews['DATE_ACTIVE_FROM'] 	= $arEA['DATE_ACTIVE_FROM'];
            $arNews['PREVIEW_TEXT'] 		= $arEA['PREVIEW_TEXT'];
            $arNews['PREVIEW_TEXT_TYPE'] 	= $arEA['PREVIEW_TEXT_TYPE'];
		}
