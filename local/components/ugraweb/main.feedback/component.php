	...
	if(check_bitrix_sessid())
	{
		...
		if(empty($arResult["ERROR_MESSAGE"]))
		{
			...
			if(!empty($arParams["EVENT_MESSAGE_ID"]))
			{
                ...
                {
                    $IB = \CIBlock::GetList([],['CODE'=>'feedback_messages'])->GetNext()['ID'];

                    $arProps = [];
                    if($_POST["user_name"])
                    {
                        $arProps['FIO'] = htmlspecialcharsbx($_POST["user_name"]);
                    }
                    $arProps['EMAIL'] = htmlspecialcharsbx($_POST["user_email"]);
                    $arProps['MESSAGE'] = ["TEXT" => htmlspecialcharsbx($_POST["MESSAGE"]), "TYPE" => "text"];

                    $obEl = new \CIBlockElement;
                    $CurrentDate = date('d.m.Y H:i:s');
                    $newID = $obEl->Add([
                        'IBLOCK_ID' => $IB,
                        'NAME' => 'Сообщение от ' . $CurrentDate,
                ...
                if($newID > 0)
                {
                    $arFields['LINK'] = 'http://'.$_SERVER['SERVER_NAME'].'/bitrix/admin/iblock_element_edit.php?IBLOCK_ID='.$IB.'&type=messages&ID='.$newID.'&lang=ru&find_section_section=-1&WF=Y';
                }
