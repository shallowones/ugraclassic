...
$arFilter[">=DATE_ACTIVE_TO"] = date($DB->DateFormatToPHP(CLang::GetDateFormat("SHORT")), $mCurDate);
...
while($obEvents = $rsEvents->GetNextElement())
{
    ...
    $arFields['PROPS'] = $obEvents->GetProperties();

    foreach ($arFields['PROPS']['dates']['VALUE'] as $date)
    {
        $arDate = explode(" ", $date);
